...
const char *toString(CompositeRRTStar::MetricMode mode) {
    switch (mode) {
    case CompositeRRTStar::MetricMode::Makespan:
        return "makespan";
    case CompositeRRTStar::MetricMode::PlainL2:
        return "plain_l2";
    }
    return "makespan";
}

ompl::base::PlannerStatus CompositeRRTStar::solve(double timeLimit) {
    ...
    setup.getProblemDefinition()->setIntermediateSolutionCallback(
        [&](const ob::Planner *, const std::vector<const ob::State *> &states,
            const ob::Cost cost) {
            auto paths = splitCompositeStates(*problem_, blocks, states);
            events.push_back(
                makeEventJson(solve_start, paths, cost.value(),
                              "composite_rrtstar_improvement"));
        });
...
std::vector<Path> CompositeRRTStar::getSolutionPaths() const {
