...
bool expectTrue(const std::string &label, bool value) {
    if (!value) {
        std::cerr << "composite_rrtstar_regression: " << label << "\n";
        return false;
    }
    return true;
}
...
bool solveAndCheck(comotion::CompositeRRTStar &planner,
    ...
    if (status != ob::PlannerStatus::EXACT_SOLUTION) {
        std::cerr << "composite_rrtstar_regression: expected exact solution, got "
                  << status.asString() << "\n";
        return false;
    }
...
bool testDefaultMakespanMetricAndEndpoints() {
    comotion::CompositeRRTStar planner;
...
bool testPlainL2MetricModeAndBoundedSimplification() {
    comotion::CompositeRRTStar planner;
    planner.setMetricMode(comotion::CompositeRRTStar::MetricMode::PlainL2);
...
int main() {
    ...
    std::cout << "composite_rrtstar_regression: OK\n";
    return 0;
}
